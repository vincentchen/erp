...
if (isset($_POST['MoveGroup'])) {
	$sql="UPDATE chartmaster SET group_=? WHERE group_=?";
...
if (isset($_POST['submit'])) {
	...
	$sql="SELECT count(groupname)
			FROM accountgroups
			WHERE groupname=? ";
	$Parameters=array($_POST['GroupName']);
	...
	if ($_POST['ParentGroupName'] !=''){
		if (CheckForRecursiveGroup($_POST['GroupName'],$_POST['ParentGroupName'],$db)) {
			...
			$sql = "SELECT pandl,
					...
					FROM accountgroups
					WHERE groupname=?";
			$Parameters = array($_POST['ParentGroupName']);
			$DbgMsg = _('The SQL that was used to retrieve the information was');
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'ChartMaster'
	...
	$sql= "SELECT COUNT(group_) AS groups FROM chartmaster WHERE chartmaster.group_=?";
	...
	if ($myrow['groups']>0) {
		...
		$sql = "SELECT COUNT(groupname) groupnames FROM accountgroups WHERE parentgroupname=?";
		...
		if ($myrow['groupnames']>0) {
			...
			$sql="DELETE FROM accountgroups WHERE groupname=?";
...
if (!isset($_GET['Delete'])) {
	...
	$SectionResult = DB_query($sql, $db,$ErrMsg,$DbgMsg);
	while( $SectionRow = DB_fetch_array($SectionResult) ) {
		if ($_POST['SectionInAccounts']==$SectionRow['sectionid']) {
			echo '<option selected="selected" value="'.$SectionRow['sectionid'].'">' . $SectionRow['sectionname'].' ('.$SectionRow['sectionid'].')</option>';
		} else {
			echo '<option value="' . $SectionRow['sectionid'] . '">' . $SectionRow['sectionname'].' ('.$SectionRow['sectionid'].')</option>';
		}
	}
	echo '</select></td>
		</tr>
