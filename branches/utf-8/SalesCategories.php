...
$CategoryPath = '<a href="'.$_SERVER['PHP_SELF'] . '?' . SID . 
			'&ParentCategory=0">' . htmlentities(_('Main'), ENT_QUOTES, _('UTF-8')) . '</a>' . "&nbsp;\\&nbsp;";
$TempPath = '';
...
for($Buzy = (isset($TmpParentID) && ($TmpParentID <> '')); 
	...
	if( $result ) {
		if (DB_num_rows($result) > 0) {
			...
			$LastParentName =  htmlentities($row['salescatname'], ENT_QUOTES, _('UTF-8'));
	...
	while( $myrow = DB_fetch_array($result) ) {
		if ( !array_keys( $stockids, $myrow['stockid']  ) ) {
			...
			echo '<option value="'.$myrow['stockid'].'">'.
				htmlentities($myrow['stockid'], ENT_QUOTES, _('UTF-8')) .
				'&nbsp;-&nbsp;&quot;'.
				htmlentities($myrow['description'], ENT_QUOTES, _('UTF-8')) . '&quot;';
		}
	}
	echo '</select>';
	echo '</td></tr></table>';
...
if($result ) {
	if( DB_num_rows($result)) {
		...
		while( $myrow = DB_fetch_array($result) ) {
			...
			echo '<td>' . htmlentities($myrow['stockid'], ENT_QUOTES, _('UTF-8')) . '</td>';
			echo '<td>' . htmlentities($myrow['description'], ENT_QUOTES, _('UTF-8')) . '</td>';
